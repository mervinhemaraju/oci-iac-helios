...
locals {
  ...
  networking = {
    ...
    gateways = {
      ...
      gaia_lpg          = jsondecode(data.doppler_secrets.oci_creds.map.OCI_GAIA_CONNECTIONS)["lpg"]["helios_dev_id"]
      rpc_id_poseidon   = jsondecode(data.doppler_secrets.oci_creds.map.OCI_POSEIDON_CONNECTIONS)["rpc"]["helios_dev_id"]
    }
