...
# * Create a private subnet for db
resource "oci_core_subnet" "db_private" {
  ...
  display_name               = "subnet-private-db"
  dns_label                  = "privatedb"
  prohibit_public_ip_on_vnic = true
  route_table_id             = oci_core_vcn.web.default_route_table_id
  security_list_ids          = [oci_core_vcn.web.default_security_list_id, oci_core_security_list.db_main.id]

  depends_on = [
    oci_core_vcn.web,
    oci_core_security_list.db_main
  ]
}
